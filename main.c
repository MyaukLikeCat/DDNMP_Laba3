...
void LED_ON(uint16_t num_LED);                                           // Функция зажигает светодиоды
...
int main(void)
{
	...
	RCC->APB2ENR = RCC->APB2ENR | RCC_APB2ENR_IOPAEN_Msk; // Для порта А
	RCC->APB2ENR = RCC->APB2ENR | RCC_APB2ENR_IOPCEN_Msk; // Для порта С
    
    /*Настройка порта для светодиодов. Режим записи*/
    for (uint16_t i = 0; i <= 7; i++)
    {
        auto_PIN_on(GPIOA, i, 2);
    }
    
    /*Настройка порта для тумблеров. Режим чтения*/    
    for (uint16_t i = 0; i <= 4; i++)
    {
        auto_PIN_on(GPIOC, i, 0);
    } 
  ...
  while (1)
  {
      ...
      switch(state)
      {
          ...
          case 1:                        // LOAD - загрузка значения с тумблера
          {
              cd = LOAD_TOAGLE();
              pb = 0;
              break;
